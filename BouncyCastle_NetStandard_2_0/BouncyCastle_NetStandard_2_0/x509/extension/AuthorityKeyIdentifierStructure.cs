    ...
    public class AuthorityKeyIdentifierStructure : AuthorityKeyIdentifier
    {
            ...
            X509Certificate certificate)
        {
            try
            {
                ...
                SubjectPublicKeyInfo info = SubjectPublicKeyInfoFactory.CreateSubjectPublicKeyInfo(certificate.GetPublicKey());

                return (Asn1Sequence)new AuthorityKeyIdentifier(info, new GeneralNames(genName), certificate.SerialNumber).ToAsn1Object();
            }
            catch (Exception e)
            {
